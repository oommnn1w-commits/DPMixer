...
class Dataset_M4(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', target='OT', seasonal_patterns='Yearly', **kwargs):
        """
        root_path: 数据集路径
        flag: 'train' 或 'test'
        size: [seq_len, label_len, pred_len]
        features, target: 保留以兼容 data_provider
        seasonal_patterns: M4 频率
        **kwargs: 接收多余参数，避免报错
        """
        self.root_path = root_path
        self.flag = flag
        ...
        self.pred_len = size[2] if size is not None else M4Meta.horizons_map[seasonal_patterns]
    ...
    def __read_data__(self):
        ...
        self.ids = np.array(dataset.ids[mask])
    ...
    def last_insample_window(self):
        """
        返回所有时间序列的最后一个 insample 窗口，用于验证/推理
        Shape: [num_series, seq_len]
        """
